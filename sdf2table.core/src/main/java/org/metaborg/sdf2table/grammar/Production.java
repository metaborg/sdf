 ...
 * 
 */
public class Production extends IProduction{
	/**
	 ...
	 */
	public Production(NonTerminal symbol, String cons, List<Symbol> rhs, Set<Attribute> attributes){
		_symbol = symbol;
		_rhs = rhs;
		_cons = cons;
	...
	public boolean merge(Production p){
	...
	public static Production fromATerm(IStrategoTerm term, Syntax syntax){
		if(term instanceof IStrategoAppl){
			...
			if(app.getName().equals("SdfProduction") || (with_cons = true && app.getName().equals("SdfProductionWithCons"))){
				...
				return syntax.uniqueProduction(new Production(symbol, cons, sym_list, attrs));
			}
		}
		return null;
	}
