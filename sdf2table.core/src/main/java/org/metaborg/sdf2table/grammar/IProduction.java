...
public abstract class IProduction{
	/**
	...
	public void computeDependencies(){
		...
		if(isEmpty()){
			...
			if(sym instanceof Terminal){
				...
				for(IProduction p : ((NonTerminal)sym).productions()){
		...
		for(int i = 0; i < size(); ++i){
			...
			if(sym instanceof NonTerminal){
				...
				for(IProduction sp : nt.productions()){
					if(!shallowConflicts(sp, i)){
						if(next == null){
							...
							if(next instanceof Terminal){
								...
								for(IProduction np : ((NonTerminal)next).productions()){
	...
	public abstract Production asProduction();
	 ...
	 */
	public boolean shallowConflicts(IProduction np, int pos){
		...
		if(np.product().nonContextual().equals(next.nonContextual())){
			return asProduction().priorities().shallowConflicts(np.asProduction(), pos);
		}
		return false;
	}
