                ...
                for(IStrategoTerm restriction : restrictions) {
                    if(restriction instanceof StrategoAppl) {
                        ...
                        switch(ares.getName()) {
                            ...
                            default:
                                System.err
                                    .println("Restrictions Section: Unknown restriction `" + ares.getName() + "'.");
                                break;
                        }
                    } else {
                ...
                for(IStrategoTerm chain : chains) {
                    if(chain instanceof StrategoAppl && ((StrategoAppl) chain).getName().equals("Chain")) {
                        ...
                        for(IStrategoTerm group : groups) {
                            ...
                            if(group instanceof StrategoAppl
                                && ((StrategoAppl) group).getName().equals("NonTransitive")) {
                                a.transitive = false;
                                group = group.getSubterm(0);
                            }
                            if(group instanceof StrategoAppl
                                && ((StrategoAppl) group).getName().equals("WithArguments")) {
                                StrategoAppl argv = (StrategoAppl) group.getSubterm(1);
                                if(!argv.getName().equals("Default")) {
                                    System.err
                                        .println("Priorities Section: Malformed term: Unknown priority parameter `"
                                            + argv.toString() + "'");
                                    continue;
                                }
                                ...
                                if(positions.size() > 1) {
                                    System.err.println(
                                        "Priorities Section: Malformed term: Unsuported multi positions priority"); // TODO
                                                                                                                    // multi-positions
                                                                                                                    // priority
                                                                                                                    // ?
                                    continue;
                            ...
                            if(group instanceof StrategoAppl
                                && ((StrategoAppl) group).getName().equals("SimpleGroup")) {
                                a.priorities.add(new Priority(importProduction(syntax, group.getSubterm(0)), a.position,
                                    a.transitive));
                            } else if(group instanceof StrategoAppl
                                && ((StrategoAppl) group).getName().equals("ProdsGroup")) {
                                for(IStrategoTerm tp : (StrategoList) group.getSubterm(0)) {
                                    a.priorities
                                        .add(new Priority(importProduction(syntax, tp), a.position, a.transitive));
                                }
                            } else {
                                ...
                                if(p != null) {
                                    if(ascendant != null) {
                                        for(Priority higher : ascendant.priorities) {
                                            Priority lower =
                                                new Priority(p.production(), higher.position(), higher.isTransitive());
    ...
    public static Symbol importSymbol(SymbolCollection collection, IStrategoTerm term) {
        ...
        if(term instanceof StrategoAppl) {
            ...
            switch(app.getName()) {
                ...
                case "Alt":
                    symbol =
                        new Alternative(importSymbol(null, app.getSubterm(0)), importSymbol(null, app.getSubterm(1)));
                    break;
                case "Sequence":
                    symbol =
                        new Sequence(importSymbol(null, app.getSubterm(0)), importSymbolList(null, app.getSubterm(1)));
                    break;
                case "Lex":
                ...
                case "IterSep":
                    symbol =
                        new Iteration(importSymbol(null, app.getSubterm(0)), 1, importSymbol(null, app.getSubterm(1)));
                    break;
                case "IterStarSep":
                    symbol =
                        new Iteration(importSymbol(null, app.getSubterm(0)), 0, importSymbol(null, app.getSubterm(1)));
                    break;
                case "Start":
                    symbol = new Start();
                    break;
        ...
        if(term instanceof StrategoAppl) {
            ...
            switch(app.getName()) {
                ...
                case "Present":
                    return importTerminal(app.getSubterm(0));
                case "Range":
                    return new Range(((Numeric) importTerminal(app.getSubterm(0))).getValue(),
                ...
                case "Conc":
                    return new Union(importTerminal(app.getSubterm(0)), importTerminal(app.getSubterm(1)));
                default:
